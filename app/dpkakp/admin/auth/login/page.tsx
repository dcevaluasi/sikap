...
function page() {
    ...
    const handleLoginAdminDPKAKP = async (e: any) => {
        if (email == "" && password == "") {
            ...
            try {
                const response = await axios.post(`${process.env.NEXT_PUBLIC_DPKAKP_UJIAN_URL}/adminPusat/login`, {
                    email: email,
                    password: password,
                })
            ...
            <section className="relative h-fit space-y-6 pb-8 pt-36 md:h-screen md:pb-12 md:pt-20 lg:py-44 w-full flex items-center justify-center flex-col">
                <div className="container relative flex max-w-[64rem] flex-col items-center gap-2 text-center">
                    ...
                    <p className="font-jakarta max-w-[42rem] leading-[100%] text-mutedForegroundDPKAKP  sm:text-lg sm:leading-8 -mt-4">
                        Selamat Datang Dewan Penguji, Silahkan Login Untuk Mengakses Fitur Ujian Keahlian Awak Kapal Perikanan
                    </p>
                    <Image
                ...
                <div className="flex flex-col gap-3 w-full max-w-md mx-auto z-50">
                    <div className="flex flex-col gap-1">
                        ...
                        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className='border rounded-xl text-white border-mutedDPKAKP bg-transparent w-full' placeholder='Enter your email address' />
                    </div>
                    <div className="flex flex-col gap-1">
                        <p className="font-jakarta  leading-[100%] text-mutedForegroundDPKAKP  sm:text-sm sm:leading-8 ">
                            Password
                        </p>
                        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className='border rounded-xl text-white border-mutedDPKAKP bg-transparent w-full' placeholder='Enter your password' />
                    </div>
                    <button onClick={(e) => handleLoginAdminDPKAKP(e)} className='text-white w-full bg-blue-950 rounded-xl bg-opacity-100 py-2'>Login</button>
                </div>
            </section>
        </main>
    )
